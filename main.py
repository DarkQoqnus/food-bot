...
def toggle(update, ctx):
    ...
    update.message.reply_text(f"شنود پیام‌های گروه الان {'روشن ✅' if state['active'] else 'خاموش ❌'} است.")
...
async def sender_loop():
    ...
    while True:
        if send_queue:
            user_id, text = send_queue.popleft()
            wait = max(0, GLOBAL_RATE_SECONDS - (time.time() - LAST_SEND_AT))
...
async def group_listener(event):
    ...
    try:
        ...
        return
    if match_sale(text, state["filter_words"]) and can_dm_seller(sender_id):
